    ...
    AC::Output: ToConstraintField<ConstraintF>,
{
        ...
        HG: TwoToOneCRHGadget<H, E::Fr>,
    {
        ...
        // println!("setup prover in comtree took: {:?} in 1 running", duration);
        
        start = Instant::now();
        let proof = groth16::create_random_proof(prover, &pk.pk, rng)?;
        duration = start.elapsed();
        // println!("generate_proof in comtree took: {:?} in 1 running", duration);
...
mod test {
    ...
    fn test_com_tree_proof_poseidon() {
        ...
        // println!("generate tree proof took: {:?} in 1 running", duration);
